...
(define special-forms-table (list '*table*)) ;; list of all installed forms
(define (lookup-action x) (lookup x special-forms-table)) ;; simple 1d table
(define (install-special-form x y)(insert! x y special-forms-table) x)

(define (xeval- exp env)
  (let ((action (lookup-action exp)))
    (cond ((not (equal? action #f)) (action exp env))
          (else "Check for other exp"))))
...
;; env: (list frame1 frame2 frame3..)
;; frame: (cons variable-list value-list)
;; variable-list: (list var1 var2 var3..)
;; value-list: (list val1 val2 val3..)

(define (search-in-frame var varlist) ;; loops through a frame
  (cond ((null? varlist) #f)
        ((equal? var (car varlist)) #t)
        (else (search-in-frame var (cdr varlist)))))

(define (defined? exp framelist) ;; loops through all frames, runs search
  (cond ((null? framelist) #f)
        ((search-in-frame exp (car framelist)) #t)
        (else (defined? exp (cdr framelist)))))

(define (locally-defined? exp framelist) ;; no need for loop, just search once
  (search-in-frame exp (car framelist)))


;; loops through a frame's variables and values to find proper grouping to erase 
(define (unbind-in-frame! var varlist vallist)
  (define (deleteentry! vars vals);; erases entries of lists and moves lists
    (define (shift! x)
      (set-car! x (cadr x)) ;; move element up
      (set-cdr! x (cddr x))) ;; move list up
    (shift! vars)
    (shift! vals)) 
  (cond ((or (null? varlist) (null? vallist)) #f) 
        ((equal? var (car varlist)) (deleteentry! varlist vallist))
        (else (unbind-in-frame! var (cdr varlist) (cdr vallist)))))

;; loops through frames to find all frames where exp is defined, then erases
(define (make-unbound! exp framelist)
  (cond ((null? framelist) #f)
        ((locally-defined? exp framelist)
         (unbind-in-frame! exp
                           (frame-variables (car framelist))
                           (frame-values (car framelist))))
        (else (make-unbound! exp (cdr framelist)))))

;; no need to loop, just check first (local) frame
(define (locally-make-unbound! exp framelist)
  (if (locally-defined? exp framelist)
      (unbind-in-frame! exp (caar framelist) (cdar framelist))))
...
(define (setup-environment)
  (let ((initial-env
         (cons ;; starting enviroment:
          (cons '() '()) ;; empty frame
          the-empty-environment))) ;; will be list of other frames
    initial-env))
...
(define (install-primitive-procedures name action)
  (add-binding-to-frame! name action (car the-global-enviroment)))
